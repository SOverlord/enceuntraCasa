...
class inicioArrendador(session_module.BaseSessionHandler, webapp2.RequestHandler):
    def get(self): 
        ...
        if not self.session.get('usuario'):
            ...
            if (tipo == "arrendador"):     #Validamos que ingrese el tipo correcto de usuario
                ...
                template = template.get_template("inicioArrendador.html")            
                print(nombreS)
...
application = webapp2.WSGIApplication([('/inicioArrendador', inicioArrendador),], config = session_module.myconfig_dict, debug=True)
